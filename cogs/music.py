...
class Music(commands.Cog):
    ...
    # --- 使用 hybrid_command 讓指令同時支援 !play 和 /play ---
    
    @commands.hybrid_command(name='play', description='播放 YouTube 音樂 (支援時間跳轉，例如: 1:30)')
    async def play(self, ctx, *, search: str):
        # 由於 hybrid_command 在斜線模式下無法自動加入語音，需多做檢查
    ...
    @commands.hybrid_command(name='pause', description='暫停目前播放的音樂')
    ...
    @commands.hybrid_command(name='resume', description='恢復播放音樂')
    async def resume(self, ctx):
        if ctx.voice_client and ctx.voice_client.is_paused():
            ctx.voice_client.resume()
            await ctx.send("▶️ 音樂已恢復播放。")
        else:
            await ctx.send("音樂未被暫停。", ephemeral=True)
    ...
    @commands.hybrid_command(name='queue', description='顯示目前的播放佇列')
    async def queue(self, ctx):
        if ctx.guild.id in self.song_queue and self.song_queue[ctx.guild.id]:
            ...
            await ctx.send("播放佇列是空的。", ephemeral=True)
    ...
    async def stop(self, ctx):
        if ctx.voice_client:
            ...
            await ctx.send("⏹️ 已停止。")
             ...
             await ctx.send("機器人未在播放中。", ephemeral=True)
    ...
    async def leave(self, ctx):
        if ctx.voice_client:
            ...
            await ctx.send("我不在任何語音頻道中。", ephemeral=True)
